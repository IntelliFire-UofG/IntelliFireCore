...
void ADS1015rpi::start(ADS1015settings settings) {
	ads1015settings = settings;
	...
	thr = std::thread(&ADS1015rpi::worker,this);
}


void ADS1015rpi::setChannel(ADS1015settings::Input channel) {
	unsigned r = i2c_readWord(reg_config);
	r = r & (~(3 << 12));
	r = r | (channel << 12);
	i2c_writeWord(reg_config,r);
	ads1015settings.channel = channel;	
}


void ADS1015rpi::dataReady() {
	float v = (float)i2c_readConversion() / (float)0x7fff * fullScaleVoltage() * 2;
	for(auto &cb: adsCallbackInterfaces) {
	    cb->hasADS1015Sample(v);
	}
}


void ADS1015rpi::worker() {
    while (running) {
...
void ADS1015rpi::i2c_writeWord(uint8_t reg, unsigned data)
{
	uint8_t tmp[3];
	tmp[0] = reg;
...
int ADS1015rpi::i2c_readConversion()
{
	const int reg = 0;
	char tmp[3];
	tmp[0] = reg;
	write(fd_i2c,&tmp,1);
        ...
        return ((int)(tmp[0]) << 4) | (int)(tmp[1])>>4;
}
