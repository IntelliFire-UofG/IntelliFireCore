...
void PumpControl::start() {
    ...
    if (!pump_chip) {
        qCritical() << "PumpControl: Failed to open GPIO chip.";
        throw "PumpControl: GPIO chip open failed.";
    }
    ...
    if (!pump_out) {
        ...
        pump_chip = nullptr;
        throw "PumpControl: Failed to get pump GPIO line.";
    }

    if (gpiod_line_request_output(pump_out, "PumpControl", 0) < 0) {
        gpiod_chip_close(pump_chip);
        pump_chip = nullptr;
        pump_out = nullptr;
        throw "PumpControl: Failed to request output line.";
    }
...
void PumpControl::stop() {
    ...
    if (pump_out) {
        gpiod_line_set_value(pump_out, 0);  // Ensure pump is off
...
void PumpControl::fireDetected(unsigned int id, int event_type) {
    ...
    if (event_type == GPIOD_LINE_EVENT_RISING_EDGE) {
        fire_detected[id] = false;  // Flame disappeared
    } else if (event_type == GPIOD_LINE_EVENT_FALLING_EDGE) {
        fire_detected[id] = true;   // Flame detected
    }

    activate_pump = fire_detected[0] && fire_detected[1];

    if (activate_pump != prev_pump_status) {
        if (pump_out) {
            gpiod_line_set_value(pump_out, activate_pump ? 1 : 0);
        }
...
#ifdef DEBUG
        qDebug() << "Pump status changed: " << (activate_pump ? "ON" : "OFF");
#endif
    }
}
